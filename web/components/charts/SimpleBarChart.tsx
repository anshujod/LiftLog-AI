...
export function SimpleBarChart({ data, color = "var(--color-acid)" }: SimpleBarChartProps) {
  const interval = data.length > MAX_TICKS ? Math.ceil(data.length / MAX_TICKS) - 1 : 0;

  return (
    <ResponsiveContainer width="100%" height={180}>
      <BarChart data={data} margin={{ top: 8, right: 12, left: 12, bottom: 0 }}>
        <CartesianGrid vertical={false} stroke="var(--color-border)" strokeOpacity={0.25} />
        <XAxis
          dataKey="label"
          interval={interval}
        ...
        />
        <YAxis hide />
        <Tooltip content={<BarTooltip />} cursor={{ fill: "var(--color-border)", opacity: 0.35 }} />
        <Bar dataKey="value" fill={color} radius={[0, 0, 0, 0]} isAnimationActive={false} minPointSize={3} />
      </BarChart>
    </ResponsiveContainer>
  );
}
